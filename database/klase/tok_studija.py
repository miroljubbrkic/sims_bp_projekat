class TokStudija:
<<<<<<< HEAD
    def __init__(self, oznaka_ustanove="", oznaka_programa="", student_iz_ustanove="", struka="", indeks="", skolska_godina=0, godina_studija=0, blok=0, redni_broj_upisa=0, datum_upisa="", datum_overe="", espb_pocetni=0, espb_krajnji=0):
        self.oznaka_ustanove = oznaka_ustanove
=======
    def __init__(self, ustanova="", oznaka_programa="", student_iz_ustanove="", struka="", indeks="", skolska_godina=0, godina_studija=0, blok=0, redni_broj_upisa=0, datum_upisa="", datum_overe="", espb_pocetni=0, espb_krajnji=0):
        self.ustanova = ustanova
>>>>>>> b36dc6f1a8835fcd9c3e1c6b1d75019c20fca532
        self.oznaka_programa = oznaka_programa
        self.student_iz_ustanove = student_iz_ustanove
        self.struka = struka
        self.indeks = indeks
        self.skolska_godina = skolska_godina
        self.godina_studija = godina_studija
        self.blok = blok
        self.redni_broj_upisa = redni_broj_upisa
        self.datum_upisa = datum_upisa
        self.datum_overe = datum_overe
        self.espb_pocetni = espb_pocetni
        self.espb_krajnji = espb_krajnji